...
<base href="<?php echo $local;?>" />
	...
	href="<?php echo utilities::get_public_location($icon_resource->filename)?>" />
		<?php
        }
        ...
        if (! empty($initial['meta']))
            foreach ($initial['meta'] as $key => $value)
        ...
        foreach ($area_content as $area => $content) {
            ...
            if (! empty($content['css']))
                foreach ($content['css'] as $css) {
                    if (in_array($css, $loaded_sources['css']) !== false)
                        continue;
                    if (filter_var(utilities::url_protocol_check($css), FILTER_VALIDATE_URL))
                        echo "<link rel='stylesheet' type='text/css' href='$css' />";
                    else
            ...
            if (! empty($content['script']))
                foreach ($content['script'] as $script) {
                    if (in_array($script, $loaded_sources['script']))
                        continue;
                    if (filter_var(utilities::url_protocol_check($script), FILTER_VALIDATE_URL))
                        echo "<script type='text/Javascript' src='$script'></script>";
                    else
...
<body>
		...
		<div class="wrapper">
				...
				<div class="widget <?php echo $area_content['header']['widgets'][$idx]['class'];?>" widget-id="<?php echo $area_content['header']['widgets'][$idx]['ID'];?>">
					<?php echo $widget; ?>
				</div>
				<?php } ?>
			</div>
		...
		<main>
			<div class="wrapper">
				...
				<div class="area <?php echo $area;?>">
					...
					<div class="widget <?php echo $area_content[$area]['widgets'][$idx]['class'];?>" widget-id="<?php echo $area_content[$area]['widgets'][$idx]['ID'];?>">
						<?php echo $widget; ?>
					</div>
					<?php } ?>
				</div>
		...
		<div class="wrapper">
				...
				<div class="widget <?php echo $area_content['footer']['widgets'][$idx]['class'];?>" widget-id="<?php echo $area_content['footer']['widgets'][$idx]['ID'];?>">
					<?php echo $widget; ?>
				</div>
				<?php } ?>
			</div>
